...
parser.add_argument('--model', default='NADNet', type=str, help='choose a type of model')
    ...
    while(True):
        for i in range(0,len(y_),batch_size):
            ...
            for j in range(i,min(i+batch_size,len(y_))): #read batchsize images, which uses to train
                img = np.array(Image.open(y_[j]).convert('L'),dtype='uint8')
                img1.append(img)
            get_batch_y = img1
...
def load_images(data_path):
    images = []
    file_dictory1 = glob.glob(args.train_data+'*.bmp')
...
def train():
    ...
    if args.pretrain:   model = load_model(args.pretrain, compile=False) 
        ...
        if args.model == 'NADNet': model = models.NADNet()
